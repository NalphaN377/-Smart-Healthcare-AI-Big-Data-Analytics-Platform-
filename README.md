...
训练并激活费用预测模型（默认同时训练“已编码病例成本估算”和“待入院未来病例成本预测”两个模型）：

```powershell
.\.venv311\Scripts\python.exe scripts\train_cost_model.py --sample-per-year 50000
```

也可以用 `--mode encoded` 或 `--mode future` 单独训练其中一种模型。年度预算预测不使用患者级模型：它按指定医院或服务区域的历史病例量与次均成本趋势计算预算情景，并允许显式调整病例量与成本增长率。
...
患者、医生和管理员登录后，均可从左侧工作台进入“费用预测”，并在页面中选择模式：

1. **已编码病例估算**：三类角色均可使用；填写住院日、CCSR、APR等完整编码，估算该记录最终总成本。
2. **未来病例预测**：三类角色均可使用；只接收入院前可知信息，选择未来年度，并用历史成本趋势或用户填写的增长率调整结果。
3. **年度预算预测**：仅医生和管理员可用；选择医院或服务区域及目标年度，基于历史病例量和次均成本趋势生成总预算、病例量和次均成本情景。
4. 所有模式均可留空可选字段；模型按缺失值规则处理。未来病例与预算预测不构成报价、结算或财务承诺。
...
| POST | `/api/v2/predictions/future-cost` | 待入院病例的未来年度成本预测（三类角色） |
| POST | `/api/v2/forecasts/annual-budget` | 医院/服务区域年度预算情景（医生、管理员） |
...
- 疾病中英文别名由`config/disease_dictionary.json`统一维护。AI意图解析会把“心脏衰竭”“心衰”等别名规范化为数据库中的`Heart failure`筛选；本地规则和外部大模型均使用同一映射，回答及图表使用中英文对照标签。新增词条时需填写CCSR编码、数据库规范英文名、中文展示名和别名列表，重启后端后生效。
