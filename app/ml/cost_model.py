...
def _fetch_year(year: int, sample_size: int) -> pd.DataFrame:
    columns = ",".join(f"[{name}]" for name in [*FEATURES, TARGET])
    sql = (
...
def fetch_training_data(sample_per_year: int = 50_000) -> tuple[pd.DataFrame, int]:
    ...
    frames = [_fetch_year(year, int(sample_per_year)) for year in years]
...
def _register_model(metadata: dict) -> int:
    conn = storage.get_connection()
    try:
        cursor = conn.cursor()
        cursor.execute(
...
def train_cost_model(sample_per_year: int = 50_000) -> dict:
    ...
    storage.init_schema()
    data, holdout_year = fetch_training_data(sample_per_year)
    ...
    version = f"cost-v{data_version}-{timestamp}"
    ...
    try:
        metadata["model_id"] = _register_model(metadata)
    except Exception:
...
def _load_active() -> tuple[dict, dict]:
    import joblib
...
def _validated_features(values: dict) -> dict:
