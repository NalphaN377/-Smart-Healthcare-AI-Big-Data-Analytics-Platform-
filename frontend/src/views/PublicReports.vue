...
async function load(silent = false) {
  ...
  try {
    const previousId = Number(selected.value?.id || 0)
    reports.value = (await publicReports()).data
    const target = Number(route.query.report || previousId)
    selected.value = reports.value.find((item) => Number(item.id) === target) || reports.value[0] || null
    error.value = ''
  } catch (err) {
...
function selectReport(report) {
  ...
  router.replace({ name: 'public-reports', query: { report: report.id } })
}

async function withdrawSelected() {
  if (!selected.value || withdrawing.value) return
  if (!window.confirm(`确认撤回《${selected.value.title}》？患者和医生将无法再查看，对应通知也会删除。`)) return
  withdrawing.value = true
  try {
    await withdrawReport(selected.value.id)
    await router.replace({ name: 'public-reports' })
    await load()
  } catch (err) {
...
<template>
  <main class="page">
    ...
    <section v-else class="layout">
      <aside><button v-for="report in reports" :key="report.id" :class="{active:selected?.id===report.id}" @click="selectReport(report)"><strong>{{report.title}}</strong><small>发布于 {{report.published_at?.replace('T',' ')}}</small></button></aside>
      <article>
        <div class="article-head"><h2>{{selected.title}}</h2><button v-if="can('system:manage')" class="withdraw" :disabled="withdrawing" @click="withdrawSelected">{{ withdrawing ? '撤回中…' : '撤回发布' }}</button></div>
        <p class="notice">公开聚合数据 · 不含个人病历 · 不构成诊断或治疗建议</p><pre>{{selected.content}}</pre>
      </article>
    </section>
  </main>
</template>
